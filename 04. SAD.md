...
## Дополнение к архитектурным ограничениям (лендинг)

- Лид-магнит работает только с одним полем ввода (URL).
- Нет требований по интеграции с email-сбором в MVP.
- CTA жёстко фиксирован → редирект на страницу регистрации `/signup`.
- White-label кастомизация лендинга исключена из MVP.
- Проверить адаптивность UI (mobile-first, корректное отображение элементов).
- Обеспечить SEO-разметку (h1/h2, meta) и возможность расширяемости лендинга (блок доверия, футер).
